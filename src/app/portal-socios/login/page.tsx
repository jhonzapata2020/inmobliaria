...
import { Building2, Lock, ShieldCheck, ArrowRight, AlertCircle, KeyRound, UserCheck } from 'lucide-react';
...
function PartnerLoginForm() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error || !data.user) {
        setErrorMsg(error?.message || 'Credenciales inválidas. Por favor verifica tu correo y contraseña.');
        setLoading(false);
        return;
      }

      // Check role/user_type in public.profiles for redirection
      ...
      const role = profile?.role || profile?.user_type || 'broker';
  ...
  return (
    <div className="min-h-screen bg-[#F8F7F4] flex flex-col justify-center items-center p-4 font-sans text-[#1C1917]">
      <div className="w-full max-w-md bg-white border border-[#E5E1D8] rounded-3xl shadow-xl p-8 space-y-6">
        ...
        <div className="text-center space-y-3">
          ...
          <div>
            ...
            <p className="text-xs font-mono text-[#1E3A2F] font-bold uppercase tracking-wider mt-1">
              Portal de Socios — Iniciar Sesión
            </p>
          </div>
        </div>
        ...
        {/* Registration CTA */}
        <div className="pt-4 border-t border-[#E5E1D8] text-center space-y-3">
          <p className="text-xs text-stone-600">
            ¿Aún no tienes cuenta en el Portal de Socios?
          </p>
          <Link
            href="/portal-socios/registro"
            className="inline-flex items-center justify-center gap-1.5 text-xs font-bold text-[#1E3A2F] hover:underline"
          >
            <UserCheck className="w-4 h-4 text-emerald-600" />
            <span>Crear cuenta de Socio o Corredor Aliado</span>
          </Link>
        </div>
        ...
        <div className="text-center pt-2">
          <div className="flex items-center justify-center gap-1.5 text-[11px] text-[#1E3A2F] font-mono font-semibold">
            ...
            <span>Conexión Cifrada SSL — Supabase Auth</span>
          </div>
        </div>
